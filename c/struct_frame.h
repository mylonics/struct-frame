...
inline struct checksum_t fletcher_checksum_calculation(uint8_t *buffer, uint8_t data_length) {
  checksum_t checksum;
...
inline void msg_encode(struct_buffer *buffer, void *msg_buffer, uint8_t msg_id, uint8_t size) {
...
inline void *msg_reserve(struct_buffer *buffer, uint8_t msg_id, uint8_t size) {
...
inline void msg_finish(struct_buffer *buffer) {
  if (buffer->config.has_crc) {
    checksum_t crc = fletcher_checksum_calculation(buffer->data + buffer->crc_start_loc,
                                                   buffer->crc_start_loc - buffer->size);
    buffer->data[buffer->size++] = crc.byte1;
...
#define MESSAGE_HELPER(name, msg_size, msg_id)                                          \
  inline void name##_encode(struct_buffer *buffer, name *name##_obj) {                  \
    msg_encode(buffer, name##_obj, msg_id, msg_size);                                   \
  }                                                                                     \
  inline bool name##_reserve(struct_buffer *buffer, name **msg) {                       \
  ...
  inline void name##_finish(struct_buffer *buffer) { msg_finish(buffer); }              \
  inline name name##_get(struct_buffer *buffer) {                                       \
    name msg = *(name *)(buffer->data);                                                 \
    return msg;                                                                         \
  }                                                                                     \
  inline name name##_get_from_buffer_result(buffer_parser_result_t result) {            \
    name msg = *(name *)(result.msg_loc);                                               \
    return msg;                                                                         \
  }                                                                                     \
  inline name *name##_get_ref(struct_buffer *buffer) { return (name *)(buffer->data); } \
  inline name *name##_get_ref_from_buffer_result(buffer_parser_result_t result) {       \
