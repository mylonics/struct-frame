...
inline bool parse_default_format_validate(uint8_t *data, msg_id_len_t *msg_id_len) {
  (void)data;
  (void)msg_id_len;
  return true;
}

inline bool parse_default_format_char_for_len_id(msg_id_len_t *msg_id_len, const uint8_t c) {
  msg_id_len->msg_id = c;
  msg_id_len->len = get_message_length(c);
  return true;
}

inline parser_functions_t default_parser_functions = {parse_default_format_char_for_len_id,
                                                      parse_default_format_validate};

inline parser_functions_t *parse_char_for_start_byte(const struct_frame_config config,
                                                     const uint8_t c) {
...
inline bool parse_char(struct_buffer *pb, uint8_t c) {
...
inline bool parse_buffer(uint8_t *buffer, size_t size, buffer_parser_result_t *parser_result) {
