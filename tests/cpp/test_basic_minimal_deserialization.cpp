...
bool read_and_validate_test_data(const char* filename) {
  ...
  FrameMsgInfo decode_result = basic_minimal_validate_packet(buffer, size);
...
int main(int argc, char* argv[]) {
  ...
  if (argc != 2) {
    ...
    std::cout << "[TEST END] C++ Basic Minimal Deserialization: FAIL\n\n";
    return 1;
  }
  ...
  std::cout << "[TEST END] C++ Basic Minimal Deserialization: " << status << "\n\n";
