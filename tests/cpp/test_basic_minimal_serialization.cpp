...
bool create_test_data() {
  ...
  size_t encoded_size = basic_minimal_encode(
  ...
  std::ofstream file("cpp_basic_minimal_test_data.bin", std::ios::binary);
  if (!file) {
  ...
  // Self-validate
  FrameMsgInfo decode_result = basic_minimal_validate_packet(encode_buffer, encoded_size);
...
int main() {
  ...
  std::cout << "[TEST END] C++ Basic Minimal Serialization: " << status << "\n\n";
