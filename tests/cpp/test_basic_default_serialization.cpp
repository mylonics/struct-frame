...
bool create_test_data() {
  ...
  size_t encoded_size = basic_default_encode(
  ...
  std::ofstream file("cpp_basic_default_test_data.bin", std::ios::binary);
  if (!file) {
  ...
  // Self-validate
  FrameMsgInfo decode_result = basic_default_validate_packet(encode_buffer, encoded_size);
...
int main() {
  ...
  std::cout << "[TEST END] C++ Basic Default Serialization: " << status << "\n\n";
