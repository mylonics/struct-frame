...
def _generate(out_dir: Path) -> None:
    """Run the struct-frame generator for Python + C output."""
    ...
    if result.returncode != 0:
        ...
        _check(False, "Code generation failed")
...
def _model_base_sizes(pkg) -> None:
...
def _model_extension_flags(pkg) -> None:
...
def _model_magic_stability(pkg) -> None:
    ...
    # For OneOfExtensionMessage: magic should be non-None and not all-zero
...
def _python_roundtrip(py_dir: Path) -> None:
...
def _python_legacy_compat(py_dir: Path) -> None:
...
def _c_compiles(c_dir: Path) -> None:
    ...
    _check(result.returncode == 0, f"C smoke-test compilation failed:\n{result.stderr}")
    (c_dir / "test_c_smoke").unlink(missing_ok=True)
...
def test_wire_evolution(tmp_path: Path) -> None:
    """Wire-evolution extension field tests: model + Python round-trip + C compile."""
    # Phase 1: model-level tests (no codegen needed)
    pkg = _load_model()
    _model_base_sizes(pkg)
    _model_extension_flags(pkg)
    _model_magic_stability(pkg)

    # Phase 2: generated code tests
    out = tmp_path
    _generate(out)

    _python_roundtrip(out / "py")
    _python_legacy_compat(out / "py")
    _c_compiles(out / "c")
