...
4. Runs basic tests (fundamental message types)
...
class TestRunner:
    ...
    def _compile_language(self, lang: Language) -> bool:
        ...
        # C/C++: compile test_basic and test_extended executables
        if lang.id in ("c", "cpp"):
            success = True
            for runner in ["test_basic", "test_extended"]:
    ...
    def _run_test_runner(self, lang: Language, mode: str, profile_name: str,
        ...
        if lang.id == "csharp":
            ...
            runner_arg = f"--runner {runner_name} " if runner_name != "test_basic" else ""
    ...
    def print_summary(self) -> bool:
        ...
        # Standard tests
        basic_encode_total = len(self.results["basic_encode"])
        basic_encode_passed = sum(1 for v in self.results["basic_encode"].values() if v)
        basic_validate_total = len(self.results.get("basic_validate", {}))
        basic_validate_passed = sum(1 for v in self.results.get("basic_validate", {}).values() if v)
        basic_decode_total = len(self.results["basic_decode"])
        basic_decode_passed = sum(1 for v in self.results["basic_decode"].values() if v)
        ...
        print(f"  Basic Encode:    {colorize_count(basic_encode_passed, basic_encode_total)}")
        print(f"  Basic Validate:  {colorize_count(basic_validate_passed, basic_validate_total)}")
        print(f"  Basic Decode:    {colorize_count(basic_decode_passed, basic_decode_total)}")
        ...
        total = (gen_total + comp_total + 
                 basic_encode_total + basic_validate_total + basic_decode_total + 
                 ext_encode_total + ext_validate_total + ext_decode_total)
        passed = (gen_passed + comp_passed + 
                  basic_encode_passed + basic_validate_passed + basic_decode_passed + 
    ...
    def run(self, generate_only: bool = False, check_tools_only: bool = False,
        ...
        Args:
            ...
            profile_filter: Only test specific profiles (e.g., ['ProfileBasic']).
...
Examples:
  ...
  python run_tests.py --profile ProfileBasic  # Test only ProfileBasic
