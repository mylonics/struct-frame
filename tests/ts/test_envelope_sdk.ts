...
import {
  ...
  getMessageInfo,
} from '../generated/ts/envelope-test.structframe';
...
function main(): number {
  console.log('=== TypeScript EnvelopeSdk tests ===');
  ...
  if (frame.valid) {
    ...
    const decoded = CommandEnvelope.deserialize(frame as any, 'fixed');
  ...
  if (rawFrame.valid) {
    const rawDecoded = RawDataEnvelope.deserialize(rawFrame as any, 'fixed');
