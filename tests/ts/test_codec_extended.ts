/**
 * Test codec - Extended message ID and payload tests (TypeScript).
 */
...
// Extended message item interface
interface ExtendedMessageItem {
  type: string;
  data: any;
}

// Load extended messages from JSON following MixedMessages sequence
export function loadExtendedMessages(): ExtendedMessageItem[] {
  const possiblePaths = [
  ...
  for (const filePath of possiblePaths) {
    try {
      if (fs.existsSync(filePath)) {
        ...
        const messages: ExtendedMessageItem[] = [];
        for (const item of mixedArray) {
          const msgType = item.type;
          const msgName = item.name;
          
          const msgArr = jsonData[msgType] || [];
          const msgData = msgArr.find((m: any) => m.name === msgName);
          if (msgData) {
...
const msgClasses: { [key: string]: any } = {
 ...
 */
function createExtendedMessage(MsgClass: any, testData: any, msgType: string): any {
 ...
 */
export function encodeExtendedMessages(formatName: string): Buffer {
 ...
 */
export function decodeExtendedMessages(formatName: string, data: Buffer): { success: boolean; messageCount: number } {
  const messages = loadExtendedMessages();
