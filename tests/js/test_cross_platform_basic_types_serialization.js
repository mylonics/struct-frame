/**
 * Cross-platform basic types serialization test for JavaScript struct-frame.
...
function createTestData() {
  try {
    ...
    // Create full payload matching the BasicTypesMessage structure (204 bytes)
    ...
    const outputPath = fs.existsSync('tests/generated/js') 
      ? 'tests/generated/js/javascript_basic_types_test_data.bin'
      : 'javascript_basic_types_test_data.bin';
...
function main() {
  console.log('\n[TEST START] JavaScript Cross-Platform Basic Types Serialization');
  
  try {
    // Create JavaScript test data
    if (!createTestData()) {
      console.log('[TEST END] JavaScript Cross-Platform Basic Types Serialization: FAIL\n');
      return false;
    }

    console.log('[TEST END] JavaScript Cross-Platform Basic Types Serialization: PASS\n');
    return true;
  } catch (error) {
