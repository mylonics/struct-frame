/**
 * Test runner entry point for JavaScript - Extended message ID and payload tests.
 * 
 * Usage:
...
function runEncode(formatName, outputFile) {
  const { encodeExtendedMessages } = require('./test_codec_extended');
...
function runDecode(formatName, inputFile) {
  const { decodeExtendedMessages } = require('./test_codec_extended');
...
function main() {
  ...
  console.log(`\n[TEST START] JavaScript Extended ${formatName} ${mode}`);
  ...
  console.log(`[TEST END] JavaScript Extended ${formatName} ${mode}: ${status}\n`);
