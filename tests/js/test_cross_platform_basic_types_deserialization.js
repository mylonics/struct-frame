/**
 * Cross-platform basic types deserialization test for JavaScript struct-frame.
...
function main() {
  console.log('\n[TEST START] JavaScript Cross-Platform Basic Types Deserialization');

  const args = process.argv.slice(2);
  if (args.length !== 1) {
    console.log('  Usage: ' + process.argv[1] + ' <binary_file>');
    console.log('[TEST END] JavaScript Cross-Platform Basic Types Deserialization: FAIL\n');
    return false;
  }
  ...
  try {
    ...
    console.log('[TEST END] JavaScript Cross-Platform Basic Types Deserialization: ' + (success ? 'PASS' : 'FAIL') + '\n');
    return success;
  } catch (error) {
