/**
 * Frame format serialization test for JavaScript.
 * 
 * This test serializes data using different frame formats and saves to binary files.
 * Usage: test_frame_format_serialization.js <frame_format>
 */
"use strict";
...
function createTestData(frameFormat) {
  try {
    ...
    const outputPath = fs.existsSync('tests/generated/js') 
      ? 'tests/generated/js/javascript_' + frameFormat + '_test_data.bin'
      : 'javascript_' + frameFormat + '_test_data.bin';
...
function main() {
  console.log('\n[TEST START] JavaScript Frame Format Serialization');
  ...
  if (args.length !== 1) {
    ...
    console.log('[TEST END] JavaScript Frame Format Serialization: FAIL\n');
    return false;
  }
  ...
  try {
    ...
    console.log('[TEST END] JavaScript Frame Format Serialization: PASS\n');
    return true;
  } catch (error) {
