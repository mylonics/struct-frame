...
class EnumCGen():
    ...
    def generate(field, packageName = ""):
        ...
        packageName = StyleC.underscore(packageName)
        result += 'typedef enum _%s_%s' % (packageName, StyleC.enum_name(field.name))
        ...
        result += ' %s_%s;\n' % (packageName, StyleC.enum_name(field.name))

        result += 'typedef uint8_t %s_%s;' % (packageName, StyleC.enum_name(field.name))
        return result
...
class MessageCGen():
    ...
    def generate(msg, packageName):
        ...
        struct_name = '%s_%s' % (packageName, StyleC.struct_name(msg.name))
        result += 'typedef struct _%s {' % struct_name
        ...
        result += '\n'.join([FieldCGen.generate(f, packageName) for key, f in msg.fields.items()])
        result += '\n}'
        struct_name = '%s_%s' % (packageName, StyleC.struct_name(msg.name))
        result += ' %s;\n\n' % struct_name
    ...
    def generate(package):
        ...
        if package.enums:
            ...
            for key, enum in package.enums.items():
                yield EnumCGen.generate(enum, package.name) + '\n\n'
        ...
        if package.messages:
            ...
            for key, msg in package.sortedMessages().items():
                yield MessageCGen.generate(msg, package.name) + '\n'
            yield '\n'
        ...
        if package.messages:
            ...
            for key, msg in package.sortedMessages().items():
                name = '%s_%s' % (package.name, StyleC.struct_name(msg.name))
