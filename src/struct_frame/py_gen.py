...
class FieldPyGen():
    ...
    def generate(field):
        ...
        if field.is_enum:
            pkg_prefix = field.type_package if field.type_package else field.package
            enum_class_name = field.field_type
                ...
                if oneof.discriminator_type == "msgid":
                    ...
                    for idx, field_name in enumerate(oneof.field_order):
                        ...
                        if field.is_default_type or field.is_enum:
                            ...
                            pack_format = py_struct_format.get(field.field_type, 'B')
...
class TestPyGen():
    ...
    def generate(package, imported_packages=None):
        ...
        for key, msg in testable_messages:
            ...
            func_name = f'create_test_{camel_to_snake_case(msg.name)}'
        ...
        yield '    when message ``i`` round-trips successfully under this profile, so the\n'
        yield '    caller can verify every message is exercised by at least one profile.\n'
        ...
        yield '    if verbose:\n'
        yield '        print(f"  -> {passed}/{TEST_MESSAGE_COUNT - skipped} passed ({skipped} skipped)")\n'
        ...
        yield '    A profile passes only if every *compatible* (non-skipped) message\n'
        yield '    round-trips; a profile that skips all messages is reported honestly as\n'
        yield '    \"0/0 passed\" rather than a misleading green. In addition, every message\n'
        yield '    must round-trip under at least one profile -- a message skipped by all\n'
        ...
        yield '            all_ok = False\n'
        yield '            print(f"[FAIL] {name}: {passed}/{expected} passed ({skipped} skipped)")\n'
        yield '        elif verbose:\n'
        yield '            print(f"[OK] {name}: {passed}/{expected} passed ({skipped} skipped)")\n'
        ...
        yield '    never_tested = [i for i, t in enumerate(tested) if not t]\n'
        yield '    if never_tested:\n'
        yield '        print(f"[WARN] {len(never_tested)} message(s) skipped under every profile, not exercised by this suite (indices {never_tested})")\n'
        yield '    return all_ok\n\n'
