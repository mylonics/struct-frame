...
class TestCGen():
    ...
    def generate(package):
        ...
        # Tri-state result so the runner can tell a profile-incompatible SKIP
        # apart from a genuine PASS (a skip must NOT count as a pass).
        yield '#define SF_RT_FAIL 0\n'
        yield '#define SF_RT_PASS 1\n'
        yield '#define SF_RT_SKIP 2\n\n'
        for key, msg in testable_messages:
            structName = TestCGen._struct_name(msg)
            ...
            yield f'/* Round-trip {structName} through *p*. Returns SF_RT_PASS / SF_RT_FAIL / SF_RT_SKIP. */\n'
            yield f'static inline int sf_roundtrip_{snake}(const sf_profile_entry_t* p, bool verbose) {{\n'
            yield f'    if (!p->has_pkg_id && ({defineName}_MSG_ID > 255)) {{\n'
            yield f'        if (verbose) printf("[SKIP] {structName} (%s): msg_id > 255 needs has_pkg_id\\n", p->name);\n'
            yield f'        return SF_RT_SKIP;\n'
            yield f'    }}\n'
            ...
            yield f'    return SF_RT_PASS;\n'
            yield f'}}\n\n'

        # Master runner
        yield f'#define SF_TEST_MESSAGE_COUNT {len(testable_messages)}\n\n'
        # Runs all message tests for one profile. Profile-incompatible messages
        # are counted in *out_skipped* (NOT as passes); tested[i] is set when
        # message i round-trips so the caller can verify each is exercised somewhere.
        yield 'static inline size_t sf_run_all_tests_for_profile(const sf_profile_entry_t* p, bool* tested, size_t* out_skipped, bool verbose) {\n'
        yield '    size_t passed = 0;\n'
        yield '    size_t skipped = 0;\n'
        yield '    int rt;\n'
        for idx, (key, msg) in enumerate(testable_messages):
            snake = camel_to_snake_case(msg.name)
            yield f'    rt = sf_roundtrip_{snake}(p, verbose);\n'
            yield f'    if (rt == SF_RT_SKIP) skipped++; else if (rt == SF_RT_PASS) {{ passed++; tested[{idx}] = true; }}\n'
        yield '    *out_skipped = skipped;\n'
        yield '    if (verbose) printf("  -> %zu/%zu passed (%zu skipped)\\n", passed, (size_t)SF_TEST_MESSAGE_COUNT - skipped, skipped);\n'
        yield '    return passed;\n'
        yield '}\n\n'

        yield 'static inline bool sf_run_roundtrip_all_profiles(bool verbose) {\n'
        yield '    bool all_ok = true;\n'
        yield '    bool tested[SF_TEST_MESSAGE_COUNT > 0 ? SF_TEST_MESSAGE_COUNT : 1];\n'
        yield '    for (size_t i = 0; i < (size_t)SF_TEST_MESSAGE_COUNT; i++) tested[i] = false;\n'
        yield '    for (size_t i = 0; i < SF_TEST_PROFILE_COUNT; i++) {\n'
        yield '        const sf_profile_entry_t* p = &SF_TEST_PROFILES[i];\n'
        yield '        if (verbose) printf("\\n--- %s ---\\n", p->name);\n'
        yield '        size_t skipped = 0;\n'
        yield '        size_t passed = sf_run_all_tests_for_profile(p, tested, &skipped, verbose);\n'
        yield '        size_t expected = (size_t)SF_TEST_MESSAGE_COUNT - skipped;\n'
        yield '        if (passed != expected) {\n'
        yield '            all_ok = false;\n'
        yield f'            printf("[FAIL] %s: %zu/%zu passed (%zu skipped)\\n", p->name, passed, expected, skipped);\n'
        yield '        } else if (verbose) {\n'
        yield f'            printf("[OK] %s: %zu/%zu passed (%zu skipped)\\n", p->name, passed, expected, skipped);\n'
        yield '        }\n'
        yield '    }\n'
        yield '    /* Messages skipped under every profile are not exercised by this\n'
        yield '     * generated suite (e.g. cross-package messages the per-package\n'
        yield '     * registry cannot resolve). Reported loudly but non-fatally -- a\n'
        yield '     * profile that *attempts* a message and fails is caught above. */\n'
        yield '    for (size_t i = 0; i < (size_t)SF_TEST_MESSAGE_COUNT; i++) {\n'
        yield '        if (!tested[i]) printf("[WARN] message index %zu skipped under every profile, not exercised\\n", i);\n'
        yield '    }\n'
        yield '    return all_ok;\n'
        yield '}\n'
