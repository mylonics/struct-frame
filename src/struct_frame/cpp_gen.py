    ...
    def generate(package, imported_packages=None, equality=False):
        ...
        if imported_packages:
            for pkg_name in imported_packages:
                yield '#include "%s.structframe.hpp"\n' % pkg_name
...
class TestCppGen():
    ...
    def generate(package):
        ...
        yield '    bool skipped = false;  /* profile-incompatible: not a real pass */\n'
        yield '};\n\n'
        ...
        for key, msg in testable_messages:
            ...
            yield f'        return TestResult{{true, "{structName}", "skipped: msg_id > 255 requires has_pkg_id profile", true}};\n'
            yield f'    }} else if constexpr (Config::has_length && (MsgT::MAX_SIZE > Config::max_payload)) {{\n'
            yield f'        return TestResult{{true, "{structName}", "skipped: message exceeds profile max_payload", true}};\n'
            yield f'    }} else {{\n'
        ...
        yield ' * Profile-incompatible messages are counted in *out_skipped* (NOT as\n'
        yield ' * passes); ``tested[i]`` is set true when message i round-trips under this\n'
        yield ' * profile so the caller can verify every message is exercised somewhere.\n'
        yield ' * Returns the number of genuine passes. */\n'
        yield 'template <typename Config>\n'
        yield 'inline size_t run_all_tests(bool* tested, size_t& out_skipped, bool verbose = false) {\n'
        ...
        yield '        if (results[i].skipped) {\n'
        yield '            out_skipped++;\n'
        yield '            if (verbose) {\n'
        yield '                std::printf("[SKIP] %s: %s\\n", results[i].message_name,\n'
        yield '                           results[i].error_msg ? results[i].error_msg : "");\n'
        yield '            }\n'
        yield '        } else if (results[i].passed) {\n'
        yield '            passed++;\n'
        yield '            tested[i] = true;\n'
        yield '            if (verbose) {\n'
        ...
        yield '    \n'
        yield '    if (verbose) {\n'
        yield '        std::printf("\\nTest Results: %zu/%zu passed (%zu skipped)\\n", passed, TEST_MESSAGE_COUNT - out_skipped, out_skipped);\n'
        yield '    }\n'
        yield '    \n'
        yield '    return passed;\n'
        yield '}\n\n'
        ...
        yield '    struct ProfileEntry { const char* name; size_t (*runner)(bool*, size_t&, bool); };\n'
        ...
        yield '    /* tested[i] becomes true once message i round-trips under any profile. */\n'
        yield '    bool tested[TEST_MESSAGE_COUNT == 0 ? 1 : TEST_MESSAGE_COUNT] = {false};\n'
        yield '    for (const auto& p : profiles) {\n'
        yield '        if (verbose) std::printf("\\n--- %s ---\\n", p.name);\n'
        yield '        size_t skipped = 0;\n'
        yield '        size_t passed = p.runner(tested, skipped, verbose);\n'
        yield '        size_t expected = TEST_MESSAGE_COUNT - skipped;\n'
        yield '        if (passed != expected) {\n'
        yield '            all_passed = false;\n'
        yield '            std::printf("[FAIL] %s: %zu/%zu passed (%zu skipped)\\n", p.name, passed, expected, skipped);\n'
        yield '        } else if (verbose) {\n'
        yield '            std::printf("[OK] %s: %zu/%zu passed (%zu skipped)\\n", p.name, passed, expected, skipped);\n'
        yield '        }\n'
        yield '    }\n'
        ...
        yield '    size_t never = 0;\n'
        yield '    for (size_t i = 0; i < TEST_MESSAGE_COUNT; i++) {\n'
        yield '        if (!tested[i]) { never++; std::printf("[WARN] message index %zu skipped under every profile, not exercised\\n", i); }\n'
        yield '    }\n'
        yield '    (void)never;\n'
