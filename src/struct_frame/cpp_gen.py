...
class TestCppGen():
    ...
    def generate(package):
        ...
        for key, msg in testable_messages:
            ...
            yield f'    {{\n'
            yield f'        auto msg = create_test_{CamelToSnakeCase(msg.name)}();\n'
            yield f'        auto result = FrameParsers::ProfilingTests::run_profiling_suite<{structName}, Config>(\n'
            yield f'            msg, buffer, sizeof(buffer), FrameParsers::get_message_info);\n'
            yield f'        \n'
            yield f'        if (verbose) {{\n'
            ...
            yield f'            std::printf("  Encode aligned: %zu iterations, %zu bytes\\n", \n'
            yield f'                       result.encode_aligned.iterations, result.encode_aligned.bytes_total);\n'
            yield f'            std::printf("  Decode aligned: %zu iterations, %zu bytes\\n", \n'
            yield f'                       result.decode_aligned.iterations, result.decode_aligned.bytes_total);\n'
            yield f'        }}\n'
            yield f'    }}\n'
            yield f'    \n'
