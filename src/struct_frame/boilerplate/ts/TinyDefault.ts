...
export class TinyDefault {
        ...
        switch (this.state) {
            ...
            case TinyDefaultParserState.GETTING_PAYLOAD:
                ...
                if (this.buffer.length >= this.packet_size) {
                    // Validate checksum
                    const msg_length = this.packet_size - TinyDefault.OVERHEAD;
                    ...
                    if (ck[0] === this.buffer[this.buffer.length - 2] && ck[1] === this.buffer[this.buffer.length - 1]) {
                        ...
                        result.msg_data = new Uint8Array(this.buffer.slice(TinyDefault.HEADER_SIZE, this.packet_size - TinyDefault.FOOTER_SIZE));
                    }
                    this.state = TinyDefaultParserState.LOOKING_FOR_START;
                }
                break;
    ...
    static validate_packet(buffer: Uint8Array | number[]): FrameMsgInfo {
        ...
        const msg_length = buffer.length - TinyDefault.OVERHEAD;
        ...
        if (ck[0] === buffer[buffer.length - 2] && ck[1] === buffer[buffer.length - 1]) {
            ...
            result.msg_len = msg_length;
            result.msg_data = new Uint8Array(Array.prototype.slice.call(buffer, TinyDefault.HEADER_SIZE, buffer.length - TinyDefault.FOOTER_SIZE));
        }
