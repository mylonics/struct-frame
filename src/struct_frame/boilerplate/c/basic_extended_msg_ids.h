    ...
    switch (parser->state) {
        ...
        case BASIC_EXTENDED_MSG_IDS_GETTING_PAYLOAD:
            ...
            if (parser->buffer_index >= parser->packet_size) {
                /* Validate checksum */
                size_t msg_length = parser->packet_size - BASIC_EXTENDED_MSG_IDS_OVERHEAD;
                ...
                if (ck.byte1 == parser->buffer[parser->packet_size - 2] &&
                    ...
                    result.msg_data = parser->buffer + BASIC_EXTENDED_MSG_IDS_HEADER_SIZE;
                }
                parser->state = BASIC_EXTENDED_MSG_IDS_LOOKING_FOR_START1;
            }
            break;
...
static inline size_t basic_extended_msg_ids_encode(uint8_t* buffer, size_t buffer_size,
    ...
    if (msg_size > 0 && msg != NULL) {
        memcpy(buffer + BASIC_EXTENDED_MSG_IDS_HEADER_SIZE, msg, msg_size);
    }
...
static inline frame_msg_info_t basic_extended_msg_ids_validate_packet(const uint8_t* buffer, size_t length) {
    ...
    if (ck.byte1 == buffer[length - 2] && ck.byte2 == buffer[length - 1]) {
        ...
        result.msg_len = msg_length;
        result.msg_data = (uint8_t*)(buffer + BASIC_EXTENDED_MSG_IDS_HEADER_SIZE);
    }
