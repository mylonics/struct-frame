...
class BasicExtendedMsgIdsParserState(Enum):
...
class BasicExtendedMsgIds:
    ...
    def reset(self):
        """Reset parser state"""
        self.state = BasicExtendedMsgIdsParserState.LOOKING_FOR_START1
        self.buffer = []
        self.packet_size = 0
        self.msg_id = 0
        self.msg_length = 0
    ...
    def parse_byte(self, byte: int) -> FrameMsgInfo:
        ...
        if self.state == BasicExtendedMsgIdsParserState.LOOKING_FOR_START1:
            if byte == self.START_BYTE1:
                self.buffer = [byte]
                self.state = BasicExtendedMsgIdsParserState.LOOKING_FOR_START2

        elif self.state == BasicExtendedMsgIdsParserState.LOOKING_FOR_START2:
            if byte == self.START_BYTE2:
                self.buffer.append(byte)
                self.state = BasicExtendedMsgIdsParserState.GETTING_MSG_ID
        ...
        elif self.state == BasicExtendedMsgIdsParserState.GETTING_MSG_ID:
            self.buffer.append(byte)
            self.msg_id = byte
            self.state = BasicExtendedMsgIdsParserState.GETTING_LENGTH

        elif self.state == BasicExtendedMsgIdsParserState.GETTING_LENGTH:
            self.buffer.append(byte)
            self.msg_length = byte
            self.packet_size = self.OVERHEAD + self.msg_length
            self.state = BasicExtendedMsgIdsParserState.GETTING_PAYLOAD
    ...
    def validate_packet(buffer: Union[bytes, List[int]]) -> FrameMsgInfo:
        ...
        if len(buffer) < BasicExtendedMsgIds.OVERHEAD:
            return result
        ...
        msg_length = len(buffer) - BasicExtendedMsgIds.OVERHEAD
        ...
        if ck[0] == buffer[-2] and ck[1] == buffer[-1]:
            ...
            result.msg_len = msg_length
            result.msg_data = bytes(buffer[BasicExtendedMsgIds.HEADER_SIZE:len(buffer) - BasicExtendedMsgIds.FOOTER_SIZE])
