...
class BasicExtendedLengthParserState(Enum):
...
class BasicExtendedLength:
    ...
    def reset(self):
        """Reset parser state"""
        self.state = BasicExtendedLengthParserState.LOOKING_FOR_START1
        self.buffer = []
        self.packet_size = 0
        self.msg_id = 0
        self.msg_length = 0
    ...
    def parse_byte(self, byte: int) -> FrameMsgInfo:
        ...
        if self.state == BasicExtendedLengthParserState.LOOKING_FOR_START1:
            if byte == self.START_BYTE1:
                self.buffer = [byte]
                self.state = BasicExtendedLengthParserState.LOOKING_FOR_START2

        elif self.state == BasicExtendedLengthParserState.LOOKING_FOR_START2:
            if byte == self.START_BYTE2:
                self.buffer.append(byte)
                self.state = BasicExtendedLengthParserState.GETTING_MSG_ID
        ...
        elif self.state == BasicExtendedLengthParserState.GETTING_MSG_ID:
            self.buffer.append(byte)
            self.msg_id = byte
            self.state = BasicExtendedLengthParserState.GETTING_LENGTH

        elif self.state == BasicExtendedLengthParserState.GETTING_LENGTH:
            self.buffer.append(byte)
            if len(self.buffer) == 4:
                ...
                self.state = BasicExtendedLengthParserState.GETTING_PAYLOAD

        elif self.state == BasicExtendedLengthParserState.GETTING_PAYLOAD:
            self.buffer.append(byte)
    ...
    def validate_packet(buffer: Union[bytes, List[int]]) -> FrameMsgInfo:
        ...
        if len(buffer) < BasicExtendedLength.OVERHEAD:
            return result
        ...
        msg_length = len(buffer) - BasicExtendedLength.OVERHEAD
        ...
        if ck[0] == buffer[-2] and ck[1] == buffer[-1]:
            ...
            result.msg_len = msg_length
            result.msg_data = bytes(buffer[BasicExtendedLength.HEADER_SIZE:len(buffer) - BasicExtendedLength.FOOTER_SIZE])
