# Automatically generated frame parser
# Generated by 0.0.1 at Thu Dec  4 20:27:20 2025.

from enum import Enum
from typing import Callable, List, Union
from .frame_base import (
    FrameMsgInfo, fletcher_checksum,
    validate_payload_with_crc, validate_payload_minimal,
    encode_payload_with_crc, encode_payload_minimal,
)

# =============================================================================
# BasicMinimal Frame Format
# =============================================================================

class BasicMinimalParserState(Enum):
    LOOKING_FOR_START1 = 0
    LOOKING_FOR_START2 = 1
    GETTING_MSG_ID = 2
    GETTING_PAYLOAD = 3


class BasicMinimal:
    """
    BasicMinimal - Frame format parser and encoder
    
    Format: [START_BYTE1=0x90] [START_BYTE2=0x70] [MSG_ID] [MSG...]
    """

    START_BYTE1 = 0x90
    START_BYTE2 = 0x70
    HEADER_SIZE = 3
    FOOTER_SIZE = 0
    OVERHEAD = 3

    def __init__(self, get_msg_length: Callable[[int], int] = None):
        """
        Initialize the BasicMinimal parser
        
        Args:
            get_msg_length: Callback function to get message length from msg_id
                            Required for formats without a length field
        """
        self.get_msg_length = get_msg_length
        self.reset()

    def reset(self):
        """Reset parser state"""
        self.state = BasicMinimalParserState.LOOKING_FOR_START1
        self.buffer = []
        self.packet_size = 0
        self.msg_id = 0

    def parse_byte(self, byte: int) -> FrameMsgInfo:
        """
        Parse a single byte
        
        Returns:
            FrameMsgInfo with valid=True when a complete valid message is received
        """
        result = FrameMsgInfo()

        if self.state == BasicMinimalParserState.LOOKING_FOR_START1:
            if byte == self.START_BYTE1:
                self.buffer = [byte]
                self.state = BasicMinimalParserState.LOOKING_FOR_START2

        elif self.state == BasicMinimalParserState.LOOKING_FOR_START2:
            if byte == self.START_BYTE2:
                self.buffer.append(byte)
                self.state = BasicMinimalParserState.GETTING_MSG_ID
            elif byte == self.START_BYTE1:
                self.buffer = [byte]
                self.state = BasicMinimalParserState.LOOKING_FOR_START2
            else:
                self.state = BasicMinimalParserState.LOOKING_FOR_START1

        elif self.state == BasicMinimalParserState.GETTING_MSG_ID:
            self.buffer.append(byte)
            self.msg_id = byte
            if self.get_msg_length:
                msg_length = self.get_msg_length(byte)
                if msg_length is not None:
                    self.packet_size = self.OVERHEAD + msg_length
                    self.state = BasicMinimalParserState.GETTING_PAYLOAD
                else:
                    self.state = BasicMinimalParserState.LOOKING_FOR_START1
            else:
                self.state = BasicMinimalParserState.LOOKING_FOR_START1

        elif self.state == BasicMinimalParserState.GETTING_PAYLOAD:
            self.buffer.append(byte)

            if len(self.buffer) >= self.packet_size:
                # Use shared minimal payload validation
                result = validate_payload_minimal(self.buffer, self.HEADER_SIZE)
                self.state = BasicMinimalParserState.LOOKING_FOR_START1

        return result

    def encode(self, msg_id: int, msg: bytes) -> bytes:
        """
        Encode a message with BasicMinimal format
        
        Args:
            msg_id: Message ID
            msg: Message data bytes
        
        Returns:
            Encoded frame as bytes
        """
        output = []
        output.append(self.START_BYTE1)
        output.append(self.START_BYTE2)
        # Use shared minimal payload encoding
        encode_payload_minimal(output, msg_id, msg)
        return bytes(output)

    def encode_msg(self, msg) -> bytes:
        """Encode a message object (must have msg_id and pack() method)"""
        return self.encode(msg.msg_id, msg.pack())

    @staticmethod
    def validate_packet(buffer: Union[bytes, List[int]]) -> FrameMsgInfo:
        """
        Validate a complete BasicMinimal packet in a buffer
        
        Args:
            buffer: Buffer containing the complete packet
        
        Returns:
            FrameMsgInfo with valid=True if packet is valid
        """
        if len(buffer) < BasicMinimal.OVERHEAD:
            return FrameMsgInfo()

        if buffer[0] != BasicMinimal.START_BYTE1:
            return FrameMsgInfo()
        if buffer[1] != BasicMinimal.START_BYTE2:
            return FrameMsgInfo()

        # Use shared minimal payload validation
        return validate_payload_minimal(buffer, BasicMinimal.HEADER_SIZE)
