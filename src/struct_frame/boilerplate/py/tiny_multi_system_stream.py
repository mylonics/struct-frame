...
class TinyMultiSystemStreamParserState(Enum):
    LOOKING_FOR_START = 0
    GETTING_MSG_ID = 1
    GETTING_LENGTH = 2
    GETTING_PAYLOAD = 3
...
class TinyMultiSystemStream:
    ...
    def reset(self):
        """Reset parser state"""
        self.state = TinyMultiSystemStreamParserState.LOOKING_FOR_START
        self.buffer = []
        self.packet_size = 0
        self.msg_id = 0
        self.msg_length = 0
    ...
    def parse_byte(self, byte: int) -> FrameMsgInfo:
        ...
        if self.state == TinyMultiSystemStreamParserState.LOOKING_FOR_START:
            if byte == self.START_BYTE:
                self.buffer = [byte]
                self.state = TinyMultiSystemStreamParserState.GETTING_MSG_ID

        elif self.state == TinyMultiSystemStreamParserState.GETTING_MSG_ID:
            self.buffer.append(byte)
            self.msg_id = byte
            self.state = TinyMultiSystemStreamParserState.GETTING_LENGTH

        elif self.state == TinyMultiSystemStreamParserState.GETTING_LENGTH:
            self.buffer.append(byte)
            self.msg_length = byte
            self.packet_size = self.OVERHEAD + self.msg_length
            self.state = TinyMultiSystemStreamParserState.GETTING_PAYLOAD
    ...
    def validate_packet(buffer: Union[bytes, List[int]]) -> FrameMsgInfo:
        ...
        if len(buffer) < TinyMultiSystemStream.OVERHEAD:
            return result

        if buffer[0] != TinyMultiSystemStream.START_BYTE:
            return result

        msg_length = len(buffer) - TinyMultiSystemStream.OVERHEAD
        ...
        if ck[0] == buffer[-2] and ck[1] == buffer[-1]:
            ...
            result.msg_len = msg_length
            result.msg_data = bytes(buffer[TinyMultiSystemStream.HEADER_SIZE:len(buffer) - TinyMultiSystemStream.FOOTER_SIZE])
