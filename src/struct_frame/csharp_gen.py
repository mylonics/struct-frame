...
class FileCSharpGen():
    @staticmethod
    def _file_header(package, equality=False, needs_collections=False):
    ...
    def generate_per_file(package, equality=False):
        ...
        if package.messages:
            for key, msg in package.sortedMessages().items():
                ...
                files[f'{msg.name}.cs'] = content

        # MessageDefinitions in its own file
        if package.messages:
            content = header(needs_collections=True)
    ...
    def _generate_message_definitions(package):
        ...
        result += '        {\n'
        result += '            // Find the entry for type T\n'
        result += '            foreach (var entry in _registryById.Values)\n'
        result += '            {\n'
        result += '                if (entry.PayloadType == typeof(T))\n'
        result += '                {\n'
