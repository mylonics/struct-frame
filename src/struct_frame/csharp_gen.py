...
class TestCSharpGen():
    ...
    def generate(package, namespace='StructFrame.Generated'):
        ...
        for key, msg in testable_messages:
            ...
            yield f'        // Returns RtPass / RtFail / RtSkip so the runner can tell a\n'
            yield f'        // profile-incompatible SKIP apart from a genuine PASS.\n'
            yield f'        private static int Roundtrip{structName}(ProfileEntry p, bool verbose)\n'
            yield '        {\n'
        ...
        yield '            var tested = new bool[MessageCount];\n'
        ...
        for idx, (key, msg) in enumerate(testable_messages):
            yield f'                rt = Roundtrip{msg.name}(p, verbose);\n'
            yield f'                if (rt == RtSkip) skipped++; else if (rt == RtPass) {{ passed++; tested[{idx}] = true; }}\n'
        yield '                int expected = MessageCount - skipped;\n'
        yield '                if (verbose) Console.WriteLine($"  -> {passed}/{expected} passed ({skipped} skipped)");\n'
        yield '                if (passed != expected)\n'
        yield '                {\n'
        yield '                    allOk = false;\n'
        yield '                    Console.WriteLine($"[FAIL] {p.Name}: {passed}/{expected} passed ({skipped} skipped)");\n'
        yield '                }\n'
        yield '                else if (verbose)\n'
        yield '                {\n'
        yield '                    Console.WriteLine($"[OK] {p.Name}: {passed}/{expected} passed ({skipped} skipped)");\n'
        yield '                }\n'
        yield '            }\n'
        ...
        yield '            var neverTested = Enumerable.Range(0, MessageCount).Where(i => !tested[i]).ToList();\n'
        yield '            if (neverTested.Count > 0)\n'
        yield '            {\n'
        yield '                Console.WriteLine($"[WARN] {neverTested.Count} message(s) skipped under every profile, not exercised by this suite (indices [{string.Join(\", \", neverTested)}])");\n'
        yield '            }\n'
        yield '            return allOk;\n'
        yield '        }\n\n'
