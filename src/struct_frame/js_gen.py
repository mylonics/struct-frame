...
class MessageJsClassGen():
    ...
    def generate(msg, package_name, package, packages, equality=False):
        ...
        if needs_default_bytes(msg):
            result += f'  static _defaultBytes = {ts_default_bytes_literal(msg.default_bytes)};\n'
    ...
    def _generate_unified_unpack(msg, package_msg_name):
        ...
        # and a fixed frame are different byte layouts of the same length. The
        # layout cannot be derived from the schema or the payload length alone --
        # the profile that framed the payload decides it: length-bearing profiles
        # always carry the wire form, while minimal profiles (hasLength == false)
        # carry the fixed form that encodeMessage takes from msg._buffer.
        #
        # mode="auto" therefore keeps the length heuristic: a _size payload is
        # read with the fixed decoder, anything else with the wire decoder. That
        # is correct in the only ambiguous case (_size bytes) whenever the sender
        # was a minimal profile, and it keeps minimal-profile round-trips
        # working. A caller holding a length-bearing profile's wire frame of
        # exactly _size bytes must pass mode="wire" (or call deserializeWire()).
        #
        # Exception: a discriminator-less oneof (discriminator = none) has
        # nothing on the fixed layout telling the reader which union member is
        # active, so its fixed read cannot populate the variant. Such a message
        ...
        if msg.variable:
            ...
            if wire_only_oneof:
                result += f'      // Discriminator-less variable oneof: only the wire encoding records\n'
            ...
            result += f'    }}\n'
            if wire_only_oneof:
                result += f'    // Discriminator-less variable oneof: always wire-encoded.\n'
            else:
            ...
            result += f'  static deserializeWire(buffer) {{\n'
            ...
            result += f'  static deserializeFixed(buffer) {{\n'
            result += f'    if (buffer instanceof Uint8Array && !(buffer instanceof Buffer)) {{\n'
            result += f'      buffer = Buffer.from(buffer);\n'
            result += f'    }}\n'
    ...
    def _generate_envelope_methods(msg, package_msg_name, package_name, packages):
        ...
        # Build list of valid payload types for JSDoc with field names
        payload_types = []
    ...
    def _generate_variable_methods(msg, package_msg_name):
        ...
        for key, field in msg.fields.items():
            ...
            if field.is_array and field.max_size is not None:
                ...
                if count_bytes == 2:
                    field_lines.append(f'const {name}Count = Math.min(buffer.readUInt16LE(offset), {field.max_size}); offset += 2;')
                    field_lines.append(f'msg._buffer.writeUInt16LE({name}Count, {msg_offset});')
                else:
                    field_lines.append(f'const {name}Count = Math.min(buffer.readUInt8(offset++), {field.max_size});')
                ...
                if length_bytes == 2:
                    field_lines.append(f'const {name}Len = Math.min(buffer.readUInt16LE(offset), {field.max_size}); offset += 2;')
                    field_lines.append(f'msg._buffer.writeUInt16LE({name}Len, {msg_offset});')
                else:
                    field_lines.append(f'const {name}Len = Math.min(buffer.readUInt8(offset++), {field.max_size});')
        ...
        if package.messages:
            ...
            if messages_with_id:
                ...
                if package.package_id is not None:
                    ...
                    yield ' * @returns {{size: number, minSize: number, isVariable: boolean, magic1: number, magic2: number}|undefined}\n'
                    yield ' */\n'
...
# =============================================================================
# Round-trip test code generator (JavaScript / CommonJS)
# =============================================================================

class TestJsGen():
    """Generator for JavaScript test code with dummy values for round-trip verification."""

    _PRIM_TYPES = {"uint8", "int8", "uint16", "int16", "uint32", "int32",
    ...
    def _generate_field_init(field, prefix="msg", index=0):
        ...
        if field.is_array:
            if field.size_option is not None:
                ...
                elif type_name not in TestJsGen._PRIM_TYPES and not field.is_enum:
                    items = ["{}" for _ in range(count)]
                    pad = ["{}" for _ in range(field.size_option - count)]
                else:
                    items = [TestJsGen._get_dummy_value(field, i) for i in range(count)]
                    pad = ["0" for _ in range(field.size_option - count)]
                ...
                if type_name in ("string", "bytes"):
                    ...
                    items = [TestJsGen._get_dummy_value(field, i) for i in range(num_elements)]
            ...
            # Single nested struct - the JS layout wraps it as a 1-element
            # struct array, so the setter accepts ``[{}]``.
            result += f"    {prefix}.{var_name} = [{{}}];\n"
        else:
            result += f"    {prefix}.{var_name} = {TestJsGen._get_dummy_value(field, index)};\n"

        return result

    @staticmethod
    def generate(package, imported_packages=None):
        """Generate JavaScript round-trip test code for a package."""
        ...
        if imported_packages:
            for imp in imported_packages:
                ...
                yield f"const {pascal_case(imp)}Pkg = require('./{imp_kebab}.structframe');\n"
        yield "\n"
        ...
        for key, msg in testable_messages:
            ...
            yield f'function {func_name}() {{\n'
            yield f'    const msg = new Pkg.{struct_name}();\n'
            field_index = 0
            for field_key, field in msg.fields.items():
                yield TestJsGen._generate_field_init(field, "msg", field_index)
                field_index += 1
            yield '    return msg;\n'
            yield '}\n'
        ...
        yield 'function isCompatible(MsgCls, hasPkgId, maxPayload, getInfoFn) {\n'
        ...
        yield f'const TEST_MESSAGE_COUNT = {len(testable_messages)};\n'
        yield 'module.exports.TEST_MESSAGE_COUNT = TEST_MESSAGE_COUNT;\n\n'
        ...
        yield 'function runRoundtripAllProfiles(verbose) {\n'
        ...
        yield '    return allOk;\n'
        yield '}\n'
        yield 'module.exports.runRoundtripAllProfiles = runRoundtripAllProfiles;\n\n'
